...
import static ethsigner.jsonrpc.response.JsonRpcError.CONNECTION_TO_DOWNSTREAM_NODE_TIMED_OUT;
import static ethsigner.jsonrpc.response.JsonRpcError.INTERNAL_ERROR;
import static ethsigner.jsonrpc.response.JsonRpcError.INVALID_PARAMS;
import static ethsigner.jsonrpc.response.JsonRpcError.SIGNING_FROM_IS_NOT_AN_UNLOCKED_ACCOUNT;

import java.net.SocketException;
import java.net.SocketTimeoutException;

import ethsigner.http.HttpResponseFactory;
import ethsigner.jsonrpc.JsonRpcRequest;
import ethsigner.jsonrpc.SendTransactionJsonParameters;
import ethsigner.jsonrpc.exception.JsonRpcException;
import ethsigner.requesthandler.JsonRpcRequestHandler;
import ethsigner.signing.TransactionSerialiser;
...
public class SendTransactionHandler implements JsonRpcRequestHandler {
  ...
  public void handle(final RoutingContext context, final JsonRpcRequest request) {
    ...
    try {
      ...
      context.fail(HttpResponseStatus.BAD_REQUEST.code(), new JsonRpcException(INVALID_PARAMS));
      return;
    ...
    if (senderNotUnlockedAccount(params)) {
      ...
      context.fail(
          ...
          new JsonRpcException(SIGNING_FROM_IS_NOT_AN_UNLOCKED_ACCOUNT));
      return;
    }
    ...
    try {
      ...
      if (cause instanceof SocketException || cause instanceof SocketTimeoutException) {
        ...
        context.fail(
            HttpResponseStatus.GATEWAY_TIMEOUT.code(), new JsonRpcException(INTERNAL_ERROR));
      }
    }
  }
