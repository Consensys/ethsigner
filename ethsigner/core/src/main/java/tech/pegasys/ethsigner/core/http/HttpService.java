...
public class HttpService extends AbstractVerticle {
  ...
  public HttpService(final HttpServerOptions serverOptions) {
  ...
  public void start(final Future<Void> startFuture) {
    ...
    httpServer
        ...
        .listen(
            result -> {
              if (result.succeeded()) {
                LOG.info("Http server started on {}", httpServer.actualPort());
                startFuture.complete();
              } else {
                LOG.error("Http server failed to listen", result.cause());
